...
for njets, i in zip((2,3), range(2)):
    # REVERT TO 2 JET. Generalise later.
    df = dfs[i]
    events_sig = [a for a in events if a.n_jets == njets and a.classification == 1]
    events_back = [a for a in events if a.n_jets == njets and a.classification == 0]
    ...
    # Set up BDTs and fit
    bdt_A = AdaBoostClassifier(DecisionTreeClassifier(max_depth=3, min_samples_leaf=0.01),
                               learning_rate=0.15,
                               algorithm="SAMME",
                               n_estimators=200
                               )
    ...
    for c in class_names:
        plot_data.append([a.decision_value for a in events_AB if a.process == c])
        plot_weights.append([a.event_weight * a.scale_factor for a in events_AB if a.process == c])
