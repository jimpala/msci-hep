...
#ifdef tag_metric_cxx
tag_metric::tag_metric(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      cout << "tag_metric needs to take a tree argument, champ. Terminating program.\n";
      exit(1);
   }
...
tag_metric::~tag_metric()
{
   if (!fChain) return;
   //delete fChain->GetCurrentFile();
}

Int_t tag_metric::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t tag_metric::LoadTree(Long64_t entry)
{
...
void tag_metric::Init(TTree *tree)
{
...
void tag_metric::Show(Long64_t entry)
{
...
Int_t tag_metric::Cut(Long64_t entry)
{
