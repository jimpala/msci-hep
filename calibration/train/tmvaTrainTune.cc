...
//  bool tracks = false;
  ...
  TString kindex_id = argv[3];
  TString ntrees_id = argv[4];
  TString ncuts_id = argv[5];

  TFile* outputFile = TFile::Open( "TMVA"+output_file+output_file_pt+output_file_met+trackstr+ntrees_id+"trees"+ncuts_id+"cuts"+kindex_id+"k"+".root", "RECREATE" );
...
//  TFile* testFile = new TFile("./../../../PGS/outputGeneral_fit_all_rescaling_test.root");
  ...
  */
  for(int r=0;r<signalFiles.size();r++){
    ...
    if(tracks) sig->Add(("../PGS_tracks/outputGeneral_fit_top"+signalFiles.at(r)+"_rescaling.root").c_str());
...
//    TFile* inputFileSignal = new TFile("/unix/atlasvhbb/abell/mc15data7/PGS/outputGeneral_fit_top_all_rescaling.root");
  ...
  if(tracks) inputFile = TFile::Open("../PGS_tracks/outputGeneral_fit_all_rescaling.root");
